...
def generate(format_wav):
    ...
    for i, test_path in enumerate(tqdm(testset)):
        ...
        # Получаем амплитуду и фазу входного файла 
        spectr = librosa.stft(y=audio_input,hop_length=hp.hop_length,n_fft=hp.n_fft) 
        ...
        # Получаем амплитуду и фазу сгенерированного файла 
        spectr2 = librosa.stft(y=audio_output,hop_length=hp.hop_length,n_fft=hp.n_fft) 
        #ampl2 = np.abs(spectr2) #Амплитуда
        phasa2 = np.angle(spectr2)  #Фаза      

        #подгоняем размер фаз
        p1 = phasa.shape[1]
        p2 = phasa2.shape[1]
        if p1>p2:
          delta_shape = p1-p2
          ph2 = np.pad(phasa2, ((0,0),(0,delta_shape)), mode='constant', constant_values=0)
        else:
          ph2 = np.delete(phasa2, np.s_[p1:], 1)

        ampl_phasa =  ampl*np.cos(ph2) + 1j*ampl*np.sin(ph2)
        y_ampl_phasa = librosa.istft(ampl_phasa,hop_length=hp.hop_length)

        # Сохранить в файл
        sf.write(generate_dir +'/wav_' + test_name, audio_output, hp.sr)
        sf.write(generate_dir +'/wav_phase' + test_name, y_ampl_phasa, hp.sr)
                ...
                if testset:
                    ...
                    with torch.no_grad():
                        for i, audio_input in enumerate(testset):
                            ...
                            audio_input = audio_input.squeeze().detach().cpu().numpy()
                            ...
                            sf.write(save_dir +'/wav_phase_%d_%dk_%d.wav' % (epoch, step // 1000, i), y_ampl_phasa, hp.sr)
                else:
